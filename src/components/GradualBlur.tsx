...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      baseStyle.height = responsiveHeight as string | number;
      baseStyle.width = (responsiveWidth as string | number) || "100%";
      baseStyle[config.position] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
    } else if (isHorizontal) {
      baseStyle.width =
        (responsiveWidth as string | number) ||
        (responsiveHeight as string | number);
...
(
  GradualBlurMemo as typeof GradualBlurMemo & { PRESETS: typeof PRESETS }
).PRESETS = PRESETS;
(
  GradualBlurMemo as typeof GradualBlurMemo & {
